...
MQTT 브로커를 구독하고 있다가, 센서 메시지가 오면 받아서 화면에 찍는다.

역할: [MQTT 브로커] --전달--> [이 구독자] --(화면 출력)
이번 단계([2])는 "잘 받는지"까지만 확인한다.
다음 단계([3])에서 여기에 'DB 저장' 코드를 붙이면 파이프라인이 완성된다.

전체 흐름:
    fake_publisher.py --> MQTT 브로커 --> subscriber.py(이 파일)
