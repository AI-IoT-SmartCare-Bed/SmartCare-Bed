...
우리 스키마는 '센서 하나당 한 줄'(긴 형태)이므로, 받은 JSON을
센서별로 쪼개서 여러 줄로 저장한다.
...
import json
import psycopg2
from datetime import datetime, timezone   # ★추가: 수신 시각을 서버에서 찍기 위함
...
# 박다솔이 보내는 키(왼쪽)를 우리 테이블의 sensor_code(오른쪽)로 변환.
SENSOR_MAP = {
    "fsr":         "FSR",     # 압력
    "dist_cm":     "DIST",    # 거리
    "sw420":       "SW420",   # 진동
...
def on_message(client, userdata, msg):
    ...
    # ★수신 시각을 서버에서 생성 (ESP32는 시계가 부정확하므로 서버 시각이 더 정확)
    ...
    for field, code in SENSOR_MAP.items():
        if field not in data:        # 그 센서값이 이번 메시지에 없으면 건너뜀
            continue
        value = data[field]
        cur.execute(
            "INSERT INTO sensor_reading (time, bed_id, sensor_code, value, quality) "
            "VALUES (%s, %s, %s, %s, %s)",
            (now, bed_id, code, value, 100)   # quality는 일단 100 고정
        )
